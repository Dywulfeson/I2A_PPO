...
action_space_cont = spaces.Box(low=np.array([-v_max, -v_max]),
                              high=np.array([v_max, v_max]),
                              dtype=np.float32)
speeds = 2
angles = 8
action_space_disc = spaces.Discrete(speeds * angles + 1)

self_state_dim = 7
human_state_dim = 7

human_state = spaces.Dict({
    "px": spaces.Box(low=-8, high=8, shape=()),
    "py": spaces.Box(low=-8, high=8, shape=()),
    "vx": spaces.Box(low=-v_max, high=v_max, shape=()),
    "vy": spaces.Box(low=-v_max, high=v_max, shape=()),
    "radius": spaces.Box(low=0, high=1, shape=()),
})
observation_space = spaces.Dict({
    "human_states": spaces.Dict({
        0: human_state,
        1: human_state,
        2: human_state,
        3: human_state,
    ...
    "robot_state": spaces.Dict({
        ...
        "vy": spaces.Box(low=-v_max, high=v_max, shape=()),
        "radius": spaces.Box(low=0, high=1, shape=()),
        "theta": spaces.Box(low=-np.pi / 2, high=np.pi / 2, shape=()),
        "v_pref": spaces.Box(low=v_pref, high=v_pref, shape=()),
        "gx": spaces.Box(low=-8, high=8, shape=()),
        "gy": spaces.Box(low=-8, high=8, shape=()),
    })
})
...
class BaseEnvConfig(object):
    ...
    reward.approchaing_reward = 0.1
    reward.elapsed_time_penalty = -0.02
    ...
    env.action_space = action_space_disc
    env.action_space.shape = tuple([env.action_space.n])
    env.observation_space = observation_space
    env.observation_space.shape = tuple([self_state_dim + sim.human_num * human_state_dim])
    ...
    robot.speed_samples = speeds
    robot.rotation_samples = angles
    robot.rotation_constraint = None
...
class BasePolicyConfig(object):
    ...
    '''Evaluate if all parameter are actually necessary'''
    ppo = Config()
    ppo.model_augmention = True
    ppo.full_rollout = True
    # ppo.mode = "gaussian"
    # ppo.mode = "beta"
    ppo.mode = "discrete"
    ppo.discrete_speeds = speeds
    ppo.discrete_angles = angles
    ppo.self_state_dim = self_state_dim
    ppo.human_state_dim = human_state_dim
    ppo.v_pref = 1
    ppo.hidden_dims = [150, 100, 100]
    ppo.hidden_dims_model = 64
    ppo.lam = 0.97
    ppo.action_std = 0.5
    ppo.epsilon = 0.1
    ppo.multiagent_training = True
    ppo.epochs = 2
    ppo.clip_ratio = 0.2
    ppo.max_steps_per_epoch = 300
    ppo.policy_lr = 3e-4
    ppo.value_lr = 1e-3
    ppo.target_kl = 0.01
    ppo.train_pi_iters = 80
    ppo.train_v_iters = 80
    ppo.memory_gamma = 0.98
    ppo.memory_lambda = 0.95
