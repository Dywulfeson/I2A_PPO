...
from ENVIRONMENT.envs.policy.policy_factory import policy_factory
from ENVIRONMENT.envs.utils.state import tensor_to_joint_state, JointState, transform
from ENVIRONMENT.envs.utils.action import ActionRot, ActionXY
from ENVIRONMENT.envs.utils.human import Human
from ENVIRONMENT.envs.utils.info import *
from ENVIRONMENT.envs.utils.utils import point_to_segment_dist
...
class CrowdSim(gym.Env):
    ...
    def __init__(self):
        ...
        self.elapsed_time_penalty = None
        self.approaching_reward = None
    ...
    def configure(self, config):
        ...
        self.approaching_reward = config.reward.approchaing_reward
        self.elapsed_time_penalty = config.reward.elapsed_time_penalty
        ...
        if self.current_scenario == 'circle_crossing':
            while True:
                ...
                for agent in [self.robot] + self.humans:
                    ...
                    # TODO: Zielposition nicht nachvollziehbar
                    if norm((px - agent.px, py - agent.py)) < min_dist or \
                            norm((px - agent.gx, py - agent.gy)) < min_dist: #  Warum wird hier die Zielposition verwendet?
                        collide = True
                        break
                if not collide:
                    break
            # TODO: Anfangswinkel der Roboter ändern (angle)
    ...
    def compute_movement(self, action):
        # at PPO_ALG action is a Tensor(1,)
        if self.action_mapping == None:
    ...
    def build_action_mapping(self):
        ...
        """
        speed_samples = self.config.robot.speed_samples
        rotation_samples = self.config.robot.rotation_samples
        rotation_constraint = self.config.robot.rotation_constraint

        holonomic = True if self.robot.kinematics == 'holonomic' else False
        ...
        self.action_mapping = action_space
    ...
    def reset(self, phase='test', test_case=None):
        ...
        # include joint state into env because its the proper way
    ...
    def step(self, action, update=True):
        ...
        """
        # LIMIT ACTION TO 1 VEL:
        if isinstance(action, (int, np.integer, torch.Tensor)):
            action = self.compute_movement(action)
        ...
        for i, human in enumerate(self.humans):
            ...
            ex = px + vx * self.time_step # Expected Distance between robot and enumerated human
        ...
        # check if reaching the goal and new dist
        end_position = np.array(self.robot.compute_position(action, self.time_step))
        old_dist_to_goal = norm(np.array(self.robot.get_position()) - np.array(self.robot.get_goal_position()))
        new_dist_to_goal = norm(end_position - np.array(self.robot.get_goal_position()))
        delta_to_goal = new_dist_to_goal - old_dist_to_goal
        reaching_goal = new_dist_to_goal < self.robot.radius

        reward = 0.

        # Elapsed Time Reward
        reward = reward + self.elapsed_time_penalty * self.time_step

        #Approaching Reward
        reward = reward - delta_to_goal * self.approaching_reward
        ...
        if self.global_time >= self.time_limit - 1:
            ...
            reward = reward + (dmin - self.discomfort_dist) * self.discomfort_penalty_factor * self.time_step
            done = False
